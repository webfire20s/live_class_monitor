...
<div class="page-header">
    ...
    <form action="{{ route('college.reports.export') }}" method="GET">
        @foreach($filters as $key => $value)
            @if($value) <input type="hidden" name="{{ $key }}" value="{{ $value }}"> @endif
        @endforeach
        <button type="submit" class="btn btn-primary">Export My Reports</button>
    </form>
</div>

<div class="card">
    <form action="{{ route('college.reports') }}" method="GET" class="filter-bar">
        <div>
        ...
        <table class="data-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @if(!$log->exit_time)
                            <span style="color: var(--primary-color); font-weight: 600;">● Online</span>
                        @else
                ...
                @empty
                <tr>
                    <td colspan="6" style="text-align: center; padding: 40px; color: var(--text-muted);">No logs found for your college.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
