...
<div class="page-header">
    <h1 class="page-title">Edit Student: {{ $student->name }}</h1>
    <a href="{{ route('college.students.index') }}" class="btn btn-outline">Back to List</a>
</div>

<div class="card" style="max-width: 600px;">
    <form method="POST" action="{{ route('college.students.update', $student->id) }}">
        @csrf
        @method('PUT')
        ...
        <div style="margin-bottom: 20px;">
            <label for="student_unique_id" style="display: block; font-size: 14px; font-weight: 500; margin-bottom: 8px;">Student ID</label>
            <input type="text" id="student_unique_id" name="student_unique_id" style="width: 100%; padding: 10px; border: 1px solid var(--border-color); border-radius: 6px;" value="{{ old('student_unique_id', $student->student_unique_id) }}" required>
        </div>

        <div style="margin-bottom: 20px;">
            <label for="name" style="display: block; font-size: 14px; font-weight: 500; margin-bottom: 8px;">Full Name</label>
            <input type="text" id="name" name="name" style="width: 100%; padding: 10px; border: 1px solid var(--border-color); border-radius: 6px;" value="{{ old('name', $student->name) }}" required>
        </div>

        <div style="margin-bottom: 20px;">
            <label for="email" style="display: block; font-size: 14px; font-weight: 500; margin-bottom: 8px;">Email Address (Optional)</label>
            <input type="email" id="email" name="email" style="width: 100%; padding: 10px; border: 1px solid var(--border-color); border-radius: 6px;" value="{{ old('email', $student->email) }}">
        </div>

        <div style="margin-bottom: 20px;">
            <label for="phone" style="display: block; font-size: 14px; font-weight: 500; margin-bottom: 8px;">Phone Number (Optional)</label>
            <input type="text" id="phone" name="phone" style="width: 100%; padding: 10px; border: 1px solid var(--border-color); border-radius: 6px;" value="{{ old('phone', $student->phone) }}">
        </div>

        <div style="padding: 16px; background-color: #f8fafc; border-radius: 6px; margin-bottom: 24px;">
        ...
        <button type="submit" class="btn btn-primary" style="width: 100%; padding: 12px;">Update Student</button>
    </form>
</div>
@endsection
